...
public class CardSelectionSqlStatementTest {
	...
	public void testSqlWithArchivedShown() throws ClassNotFoundException, NamingException, SQLException, MigrationFailedException, AuthorizationException {
		...
		Assert.assertEquals("SELECT CARD_ID, OWNER_ID, CONTENT, AFTER_CARD_ID, LANE_ID, BOARD_ID, USER_NAME FROM CARDS LEFT JOIN USERS ON CARDS.OWNER_ID = USERS.USER_ID WHERE LANE_ID = ?", statement.getSql());
	}
	...
	public void testSqlWithoutArchived() throws ClassNotFoundException, NamingException, SQLException, MigrationFailedException, AuthorizationException {
		...
		Assert.assertEquals("SELECT CARD_ID, OWNER_ID, CONTENT, AFTER_CARD_ID, LANE_ID, BOARD_ID, USER_NAME FROM CARDS LEFT JOIN USERS ON CARDS.OWNER_ID = USERS.USER_ID WHERE LANE_ID = ? AND IS_ARCHIVED IS ?", statement.getSql());
	}
	...
	public void testSqlWithoutArchivedTag() throws ClassNotFoundException, NamingException, SQLException, MigrationFailedException, AuthorizationException {
		...
		Assert.assertEquals("SELECT CARD_ID, OWNER_ID, CONTENT, AFTER_CARD_ID, LANE_ID, BOARD_ID, USER_NAME FROM CARDS LEFT JOIN USERS ON CARDS.OWNER_ID = USERS.USER_ID WHERE LANE_ID = ? AND IS_ARCHIVED IS ? AND CONTENT LIKE ?", statement.getSql());
	}
	...
	public void testSqlWithArchivedShownTag() throws ClassNotFoundException, NamingException, SQLException, MigrationFailedException, AuthorizationException {
		...
		Assert.assertEquals("SELECT CARD_ID, OWNER_ID, CONTENT, AFTER_CARD_ID, LANE_ID, BOARD_ID, USER_NAME FROM CARDS LEFT JOIN USERS ON CARDS.OWNER_ID = USERS.USER_ID WHERE LANE_ID = ? AND CONTENT LIKE ?", statement.getSql());
	}
}
